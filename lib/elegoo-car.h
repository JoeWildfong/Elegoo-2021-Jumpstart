...
class ElegooCar {
    ...
    void setPwr(int16_t s16Pwr, uint8_t u8ENAPin, uint8_t u8IN1Pin,
                uint8_t u8IN2Pin) {
        if (s16Pwr > 255)
            s16Pwr = 255;
        else if (s16Pwr < -255)
...
#endif
          m_i16PrevLeft(0), m_i16PrevRight(0) {
...
#if ELEGOO_HCSR04
    void setUSRate(uint16_t u16Delay) { m_US.startContinuous(u16Delay); }

    uint32_t u32GetDistance() { return m_US.getDistance(); }
#endif
