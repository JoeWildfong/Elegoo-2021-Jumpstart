...
#define DATA_EXPIRE_TIME 190

enum { ePreGame, eAutonomous, eTeleop, ePostGame };
...
class DriverStation {
  ...
  public:
    DriverStation()
        : m_u32StateChangeTime(millis()), m_u8Rcvd(0), m_bValid(false),
          m_bSlowSent(false) {}
    ~DriverStation() {}
    ...
    bool getButton(uint8_t buttonId) {
        if (buttonId >= 16) {
            Serial.print(
                "DriverStation::getButton(): Invalid Button ID requested ");
            ...
            if (m_u8Rcvd == 0) {
                ...
                if (m_u8Rcvd == sizeof(m_inBuff.gd)) {
                    ...
                    if (sum == m_inBuff.gd.u16Sum) {
                        // if the game state has changed, remember when it
                        // happend
